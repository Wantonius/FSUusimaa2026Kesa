...
export const removeItem = (token:string,id:string) => {
	return (dispatch:ThunkDispatch<AppState,any,Action>) => {
...
const handleFetch = async (request:Request,act:string,dispatch:ThunkDispatch<AppState,any,Action>,token:string) => {
		dispatch(loading());
		...
		if(response.ok) {
			...
			switch(act) {
				...
				case "removeitem":{
					dispatch(fetchItemFailed(actionConstants.REMOVE_ITEM_FAILED,
